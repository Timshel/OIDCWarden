...
- `SSO_PKCE`: Activate PKCE for the Auth Code flow (default `true`). More of a compatibility setting, see [#on-sso_pkce](#on-sso_pkce).
...
- `SSO_SYNC_ON_REFRESH`: Enable to refresh role, orgs and groups on refresh_token. Will call `user_info`, can be expensive since the client cam spam the refresh_token endpoint
...
Only control sending a PKCE challenge generated by the server to the provider. This is more a compatibility feature than a security one since it's separate from the challenge generated by the client.
Even when disabled the Bitwarden clients will send a challenge which will be verified before exchanging the code with the provider (It would be preferable to pass-it to the provider, but due to limitations with the underlying library it's not possible at the moment).
