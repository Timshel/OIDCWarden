...
// To prevent data disclosure we return a dummy value
// `get_auto_enroll_status` require authentication and will resolve a valid org if available
...
impl GroupRequest {
    ...
    pub fn update_group(&self, mut group: Group) -> Group {
        ...
        if CONFIG.sso_enabled() && CONFIG.sso_organizations_enabled() && CONFIG.org_groups_enabled() {
            group.external_id.clone_from(&self.external_id);
        }
