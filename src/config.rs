...
make_config! {
    ...
    settings {
        ...
        invitation_org_name:    String, true,   def,    "OIDCWarden".to_owned();
    ...
    sso {
        ...
        sso_organizations_token_path:   String, true,   def,    "/groups".to_owned();
    ...
    smtp: _enable_smtp {
        ...
        /// From Name
        smtp_from_name:                String, true,   def,     "OIDCWarden".to_owned();
        /// Username
...
fn validate_config(cfg: &ConfigItems, on_update: bool) -> Result<(), Error> {
    ...
    if cfg.sso_enabled {
        ...
        assert!(
            cfg.sso_organizations_id_mapping.is_empty(),
            "SSO_ORGANIZATIONS_ID_MAPPING is now REMOVED, More details: https://github.com/timshel/OIDCWarden/blob/v2025.5.0-1/README.md#deprecations"
        );
        assert!(
...
impl Config {
    ...
    pub fn sso_provider_groups_allowlist_set(&self) -> HashSet<String> {
        self.sso_provider_groups_allowlist().split(',').map(|g| g.trim().to_owned()).filter(|g| !g.is_empty()).collect()
    }
}
